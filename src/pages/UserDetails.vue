<template>
  <div class="Home">
    ...
    <div class="subject">
      ...
      <div>
        <div v-if="user_info.name == ''">
          @{{ keepFirstSixCharacters(user_info.address) }}
        </div>
        <div v-else>{{ user_info.name }}{{ If_Follow }}</div>
        <P>{{ Follow_Me.length }} followers</P>
        <P>{{ user_info.bio }}</P>
        <div v-if="If_Follow == false" @click="follow">Follow</div>
        <div v-else @click="CancelFollow">Following</div>
        <div>
    ...
    <div class="screen">
      ...
      <span @click="cut(4)" :class="{ dis: cutdetails == 4 }"
        >Coins created</span
      >
      <span @click="cut(5)" :class="{ dis: cutdetails == 5 }"
        >followers</span
      >
      <span @click="cut(6)" :class="{ dis: cutdetails == 6 }"
        >following</span
      >
    </div>
    <div class="list" v-if="cutdetails == 1">
      <div @click="toDetails(item.MintPda)" v-for="item in buylist" :key="item">
        ...
        <div>
          ...
          <div>
            ...
            <span v-if="!sjc(item.expireTime) && item.remind == 0"
              >complete</span
            >
...
let Address = ref(proxy.$route.query.address);
let id = ref(proxy.$route.query.id);
let Address2 = ref(
...
watch(
  ...
  (newValue, oldValue) => {
    if (newValue != null) {
      Address2.value = newValue.toString();
      PeopleFollow();
      FollowMe();
      localStorage.setItem("Address", Address.value);
      IfFollow();
...
let over = ref(false);
const buyinfo = () => {
  get(`/api/person/1/30/${Address.value}`, "")
    .then((res) => {
...
const establishinfo = () => {
  get(`/api/person/created/1/30/${Address.value}`, "")
    .then((res) => {
...
const decrypts = (word) => {
  return CryptoJS.AES.decrypt(word, "your-secret-key").toString(
    CryptoJS.enc.Utf8
  );
};
let If_Follow = ref(false);
const IfFollow = () => {
  if (Address2.value == null) Address2.value = localStorage.getItem("Address");
  get(`/api/fellow/judge/${Address2.value}/${Address.value}`, "")
    .then((res) => {
      If_Follow.value = res.returnData;
    })
    .catch((e) => {
      console.error(e);
    });
...
const PeopleFollow = () => {
  get(`/api/fellow/get/${Address.value}/1`, "")
    .then((res) => {
...
const FollowMe = () => {
  ...
  get(`/api/fellow/get/${Address.value}/0`, "")
    .then((res) => {
...
const follow = () => {
  ...
  if (Address2.value != null) {
    data = {
      user: encrypt(Address2.value),
      fellow: Address.value,
    };
  } else {
    data = {
